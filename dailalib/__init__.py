...
def create_plugin(*args, **kwargs):
    ...
    llm_api = LLMAPI(delay_init=True)

    # load config before creating context menus. if not, the config is only be load after "OK" button is
    # clicked in ask_settings :X
    llm_api.load_or_create_config()

    # create context menus for prompts
    gui_ctx_menu_actions = {
        f"DAILA/LLM/{prompt_name}": (prompt.desc, getattr(llm_api, prompt_name))
        for prompt_name, prompt in llm_api.prompts_by_name.items()
    }
    # create context menu for llm chat
    gui_ctx_menu_actions["DAILA/LLM/chat"] = ("Open LLM Chat...", get_llm_chat_creator(llm_api))

    # create context menus for others
    gui_ctx_menu_actions["DAILA/LLM/Settings"] = ("Settings...", llm_api.ask_settings)
    #
    ...
    llm_api.init_decompiler_interface(decompiler_interface=deci)
